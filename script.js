...
// Проверка есть ли данные в Local Storage
...
function renderTaskList() {
    ...
    doneTasks.forEach(element => renderReadyTask(element));
}
...
// Функция по отображению текущих задач на странице
...
// Обработка чекбокса о выполнении задачи
function doneTask(target) {
    ...
    // Поиск выполненной задачи во всем списке задач
...
function makeImportantTask(target) {
    ...
    currentTabData['tasks'] = currentTabData['tasks'].map(element => {
        ...
        if(element.isImportant) {
            ...
            data.important.tasks.push(element)
        }
        console.log(element)
        return element;
    })
...
function createTask() {
    ...
    createTaskInput.value = ''  //Обнулили Input
...
// Обход всех задач на проставление чекбоса и удаление
...
//Создание задачи при нажатии клавиши Enter
...
function renderSubTitle(type) { //Отображение заголовка текущей вкладки
    subTitle.innerHTML = '';
...
drawerItems.forEach(item => {
    item.addEventListener('click', () => {
        ...
        renderTaskList()
        renderSubTitle(item.lastElementChild.innerHTML)
    })
})
